...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	head();//系统头部
	login();//登录
			//DWORD acrid, anrid, exrid;
	HANDLE thread1;
	HANDLE thread2;
	mutual = CreateMutex(NULL, 0, NULL);
	//signal1 = CreateSemaphore(NULL, 1, 1, NULL);
...
void attrib(string name)
{
	...
	for (int i = 0; i < inodecount; i++)
	{
		if (inode[i].father == curdir)
		{
			...
			else if (inode[i].user_quanxian == 1)
